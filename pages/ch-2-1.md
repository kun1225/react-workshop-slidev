...
我們已經知道 React Re-render 的本質就是重新執行整個 function component」
...
export default function Form() {
  ...
  useEffect(() => {
	  setFullName(firstName + lastName) // 等 firstName 或 lastName 更新完後，再 re-render 一次來更新 fullName
  }, [firstName, lastName]) //
...
用 state 記住它 `==` 讓 React 幫你維護一份多餘的資料，還要用 useEffect 監聽依賴更新它。
...
<!--
畫面一開始會出現一閃而過的「0 0 0 0」，不管實際上還剩幾天，初始化就是 0，我不希望使用者一開始是看到 0 0 0 0。
-->
...
發現畫面<span v-mark="{color: 'var(--secondary)', at:2}">閃了不正確的值</span>或<span v-mark="{color: 'var(--secondary)', at:3}"> useState 的初始值需要預先計算</span> &rarr; 想到初始化函數 `useState(calInitState)`
</v-click>
...
這個技巧在處理倒數、API 預設資料、時間差等情境都非常好用
-->
...
你可能會想：「我都呼叫 setItems 了，為什麼畫面沒更新？」
...
<!--
所以這也引出了一個重點觀念，就是當你的 state 變複雜時，要考慮單一 state 的複雜度，不要讓 state 難以拆解
