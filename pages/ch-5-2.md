...
而且 memberLevel 是個字串，也就是原始值，所以他沒有參考值的問題，只要值不改變，useEffect 也不會重新執行。
...
<!--
可以發現就算把 `useMemo` 拿掉，由於他回傳是一個原始值，只要值不改變，useEffect 也不會重新執行。
-->
...
然後他也只是很單純的傳給 button 的 onClick 而已，ㄇ
...
5. `useCallback` 只有兩種使用場景:
   1. 函數用在 Hook 的依賴陣列中
   2. 函數被當作 Props 傳給被 `React.memo` 包住子組件時時
