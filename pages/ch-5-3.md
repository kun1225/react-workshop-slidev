...
<!--
在上一節，我們提到了「當子元件使用了 `React.memo` 包裝時，就需要記憶化傳入的 props」，這裡我們就來深入了解：React.memo 到底是什麼？又該怎麼正確使用？
-->
...
1. React 提供的高階元件（Higher-Order Component）
2. 作用是「記憶化整個元件的輸出結果」
...
[click]
就像以下程式碼，當父組件重新渲染時，React 會比對用 `React.memo` 包裝的子組件的 props 是否相同。
...
每次 re-render 時，`data` 都是一個新的物件，代表參考值改變了。
<br/>
對 React 而言，就代表 props 改變了，於是 `MemoChild` 還是會重新渲染。
</v-click>

<!--
很多時候我們的 `React.memo` 看起來沒用，是因為我們傳入的 props 是「參考值」
...
`<p>child</p>` 是 `React.createElement` 的語法糖，它會回傳一個物件的
...
`MemoParent` 的 `children` 每次都是一個新的參考值，所以 `MemoParent` 會重新渲染
...
所以 MemoParent 的 children prop 每次都是不同的參考值
-->
...
- 不要直接把 inline 的物件、陣列、函式、JSX 寫進 props
- `children` 本質上是 React element（物件），每次 render 都是新參考
- 即使子元件是 `memo` 過的，若其 JSX 寫在父層 render 中，仍會造成 re-render
