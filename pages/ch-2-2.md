...
form 表單是最適合這種方式的，尤其是在不需要複雜驗證機制的情況下，原因是我們能透過 event 物件來拿到資料，不需要額外設 state 來檢查：
...
再使用一些 UI library（像 shadcn）時要注意
...
<Select
  name="sortBy"
  onValueChange={updateQueryParams} // ❌ 這裡就抓不到 name
/>
```
...
<!--
再使用一些 UI library（像 shadcn）時：以 shadcn 的 select 來看

可以看到，shadcn select 的 onValueChange 只回傳一個 value，而不是 event object，也就拿不到 name 跟 value，所以這個寫法就會出錯
-->
...
[click]
這裡我是直接修改 shadcn 的 select 組件的 onValueChange，讓他回傳一個假的 event object 以此來統一接口
-->
...
(如果要直接傳 formData 到後端，Content-Type 要用 multipart/form-data，有人問再說)
...
但回過頭來想，歷史紀錄根本不影響畫面呈現，只是儲存操作順序而已。（切頁面）
-->
